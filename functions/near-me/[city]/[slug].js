...
export async function onRequestGet(context) {
  ...
  const [mediaRes, reviewsRes, productsRes] = await Promise.all([
    ...
    fetch(
      `${SB_URL}/rest/v1/products?listing_id=eq.${l.id}&source=eq.artist&order=sort_order.asc&select=*`,
      { headers: { 'apikey': SB_KEY, 'Authorization': `Bearer ${SB_KEY}` } }
    )
  ]);
...
function openProduct(idx){
  ...
  if(p.sell_mode==='external'&&p.external_url){
    h+='<a class="sp-buy" href="'+escH(p.external_url)+'" target="_blank" rel="noopener" style="text-decoration:none;display:inline-flex;align-items:center;justify-content:center;gap:6px" onclick="event.stopPropagation()">Shop Now \\u2192</a>';
    h+='<p style="font-size:.6875rem;color:#888;margin-top:4px;text-align:center">You\\u2019ll be redirected to the seller\\u2019s store</p>';
  }else{
